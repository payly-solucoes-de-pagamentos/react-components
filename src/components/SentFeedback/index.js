...
const SentFeedback = ({ color, success, error, size, strokeWidth }) => {
  let SetPath = <LoaderCircle cx="45" cy="45" r="42" stroke={color} strokeWidth={strokeWidth} />;
  if (success) {
    SetPath = (
  ...
  return (
    <svg version="1.1" viewBox="-8 -8 100 100" width={size} height={size} xmlns="http://www.w3.org/2000/svg">
      {SetPath}
    </svg>
  )
}
