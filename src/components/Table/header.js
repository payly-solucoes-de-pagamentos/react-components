...
export const headersToComponent = (data,
  translateEnum,
  sorts,
  setTableData,
  setSortedBy,
  ...
  dataSample = Object.keys(dataSample).filter(datakey => translateEnum[datakey]);

  const sortData =  (dataKey) => {
    const handlerData = [ ...data ];
    sortedBy === dataKey ? setTableData(handlerData.reverse()) : setTableData(data.sort(sorts[dataKey].sortStrategy));
    setSortedBy(dataKey);
  }
...
export const actionHeader = () => {
  return (
    <Styled.TableTitle width="80px" padding="12px" key="action-header">Ações</Styled.TableTitle>
  )
}
