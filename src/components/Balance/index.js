...
const Balance = React.memo(
  ({
    balance,
    isLoading,
    previsionDaysOptions,
    previsionDays,
    onPrevisionDaysSelectChange
  }) => {
    const LOW_FUNDS_MESSAGE = [
      <p key="low-p-1">Cuidado, seu saldo agendado está muito</p>,
      <p key="low-p-2">próximo do seu saldo atual</p>
    ];
    const NEGATIVE_FUNDS_MESSAGE = [
      <p key="negative-p-1">Seu saldo agendado é superior ao seu saldo.</p>,
      <p key="negative-p-2">
        Faça uma TED de {formatPrice(balance.scheduled - balance.current)}
      </p>
    ];
    ...
    const isFundsLow = () => {
      ...
      return (
        balance.scheduled * (FUNDS_LOW_PERCENTAGE / 100) >=
        balance.current - balance.scheduled
      );
    };
