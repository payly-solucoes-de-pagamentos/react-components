  ...
  if (labelLimitator) {
    return (
      <>
        ...
        <Style.LabelCounter labelLimitator={labelLimitator}>{labelLimitator && (value.length > 1 && `+${value.length - 1}`)}</Style.LabelCounter>
      </>
    );
  }
...
const MultiValueLabelHandler = (props) => {
  const customProp = props.labelLimitator ? Object.assign({ ...props }, { children: stringLimitator(props.children, 7) }) : props;
...
const SelectComponents = ({ id, value, name, onChange, placeholder, isMultiple, options, error, basicStyle, isSearchable, labelLimitator }) => {

  const MultiValueContainer = (props => <MultiValueContainerHandler {...props} labelLimitator={labelLimitator} />);
  const MultiValueLabel = (props => <MultiValueLabelHandler {...props} labelLimitator={labelLimitator} />)
  ...
  return (
    <Style.MultiSelect
      ...
      components={{ Option, IndicatorSeparator, IndicatorsContainer, MultiValueContainer, MultiValueLabel }}
      arrowRender={() => {
        return (<div>T</div>)
      }}
    />
